...
var _ meta.Consensus = (*IndexedMeta)(nil)

// IndexedMeta performs events ordering and detects cheaters
...
// New creates IndexedMeta instance.
func NewIndexedMeta(store *Store, input EventSource, dagIndexer DagIndexer, crit func(error), config Config) *IndexedMeta {
	p := &IndexedMeta{
		Meta:          NewMeta(store, input, dagIndexer, crit, config),
		dagIndexer:    dagIndexer,
...
func (p *IndexedMeta) Build(e dag.MutableEvent) error {
...
func (p *IndexedMeta) Process(e dag.Event) (err error) {
	defer p.dagIndexer.DropNotFlushed()
	err = p.dagIndexer.Add(e)
	if err != nil {
		return err
...
func (p *IndexedMeta) Bootstrap(callback meta.ConsensusCallbacks) error {
	base := p.Meta.OrdererCallbacks()
	...
	return p.Meta.BootstrapWithOrderer(callback, ordererCallbacks)
}
