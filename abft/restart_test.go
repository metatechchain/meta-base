...
func testRestart(t *testing.T, weights []pos.Weight, cheatersCount int) {
	...
	for _, _lch := range lchs {
		lch := _lch // capture
		lch.applyBlock = func(block *meta.Block) *pos.Validators {
	...
	for n, e := range ordered {
		if r.Intn(10) == 0 || n%20 == 0 {
			...
			restored := NewIndexedMeta(store, prev.input, &adapters.VectorToDagIndexer{vecfc.NewIndex(prev.crit, vecfc.LiteConfig())}, prev.crit, prev.config)
			assertar.NoError(restored.Bootstrap(prev.callback))

			lchs[RESTORED].IndexedMeta = restored
		}
...
func compareStates(assertar *assert.Assertions, expected, restored *TestMeta) {
	assertar.Equal(
...
func compareBlocks(assertar *assert.Assertions, expected, restored *TestMeta) {
