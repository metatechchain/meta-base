...
import (
	...
	"github.com/metatechchain/meta-base/inter/dag/tdag"
...
func TestMetaRandom_1(t *testing.T) {
	testMetaRandom(t, []pos.Weight{1}, 0)
}

func TestMetaRandom_big1(t *testing.T) {
	testMetaRandom(t, []pos.Weight{math.MaxUint32 / 2}, 0)
}

func TestMetaRandom_big2(t *testing.T) {
	testMetaRandom(t, []pos.Weight{math.MaxUint32 / 4, math.MaxUint32 / 4}, 0)
}

func TestMetaRandom_big3(t *testing.T) {
	testMetaRandom(t, []pos.Weight{math.MaxUint32 / 8, math.MaxUint32 / 8, math.MaxUint32 / 4}, 0)
}

func TestMetaRandom_4(t *testing.T) {
	testMetaRandom(t, []pos.Weight{1, 2, 3, 4}, 0)
}

func TestMetaRandom_3_1(t *testing.T) {
	testMetaRandom(t, []pos.Weight{1, 1, 1, 1}, 1)
}

func TestMetaRandom_67_33(t *testing.T) {
	testMetaRandom(t, []pos.Weight{33, 67}, 1)
}

func TestMetaRandom_67_33_4(t *testing.T) {
	testMetaRandom(t, []pos.Weight{11, 11, 11, 67}, 3)
}

func TestMetaRandom_67_33_5(t *testing.T) {
	testMetaRandom(t, []pos.Weight{11, 11, 11, 33, 34}, 3)
}

func TestMetaRandom_2_8_10(t *testing.T) {
	testMetaRandom(t, []pos.Weight{1, 2, 1, 2, 1, 2, 1, 2, 1, 2}, 3)
}

// TestMeta 's possibility to get consensus in general on any event order.
func testMetaRandom(t *testing.T, weights []pos.Weight, cheatersCount int) {
	assertar := assert.New(t)

	const lchCount = 3
	nodes := tdag.GenNodes(len(weights))

	lchs := make([]*TestMeta, 0, lchCount)
	inputs := make([]*EventStore, 0, lchCount)
	for i := 0; i < lchCount; i++ {
		lch, _, input := FakeMeta(nodes, weights)
...
func compareResults(t *testing.T, lchs []*TestMeta) {
	assertar := assert.New(t)
